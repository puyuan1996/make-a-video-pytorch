...
class PseudoConv3d(nn.Module):
    ...
    def forward(
        ...
        # x.shape: 'b c f h w -> (b f) c h w'  (1, 256, 8, 16, 16) -> (8, 256, 16, 16)
        # 其中 256 是特征维度，（16,16） 是空间维度
        ...
        # x.shape: 'b c f h w -> (b h w) c f'  (1, 256, 8, 16, 16) -> (256, 256, 8)
        # 其中 256 是特征维度，8 是时间维度（图像帧维度）
...
class SpatioTemporalAttention(nn.Module):
    ...
    def forward(
        ...
        # x.shape: 'b c f h w -> (b f) (h w) c'  (1, 256, 8, 16, 16) -> (8, 256, 256)
        # 分别对应于 attention_layer中的 (B, N, D)  B (batch), N(sequence length), D(feature dimension)
        # 即将图像的长宽维度（空间维度）作为序列长度，将通道数作为特征维度，因此称为 spatial attention
        # space_rel_pos_bias shape: 'B, N, N' -> (8, 256, 256)
        x = self.spatial_attn(x, rel_pos_bias=space_rel_pos_bias) + x
        # x.shape: '(b f) (h w) c -> b c f h w' (8, 256, 256) -> (1, 256, 8, 16, 16)
        ...
        if enable_time:
            ...
            # x.shape: 'b c f h w -> (b h w) f c'  (1, 256, 8, 16, 16) -> (256, 8, 256)
            # 分别对应于 attention_layer 中的 (B, N, D)  B (batch), N(sequence length), D(feature dimension)
            # 即将图像的帧（时间维度）作为序列长度，将通道数作为特征维度，因此称为 temporal attention
            # space_rel_pos_bias shape: 'B, N, N' -> (8, 8, 8)
            x = self.temporal_attn(x, rel_pos_bias=time_rel_pos_bias) + x
            # x.shape: '(b h w) f c -> b c f h w' (256, 8, 256) -> (1, 256, 8, 16, 16)
